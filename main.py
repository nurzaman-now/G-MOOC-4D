...
def register_user():
    try:
        ...
        if len(new_embeddings) > 5:
            ...
            if os.path.exists(root_dir):
                shutil.rmtree(root_dir)
            print("tidak ada wajah atau data wajah minimal harus 6")
            return jsonify("tidak ada wajah atau data wajah minimal harus 6"), 400
...
def delete():
    try:
        ...
        if faceId == None:
            json = request.get_json("faceId")
            faceId = json["faceId"]

        path = os.path.join(base_path, "uploads")
        if os.path.exists(path + f"/{faceId}"):
            shutil.rmtree(path + f"/{faceId}")
            if any(os.listdir("uploads")):
                refresh_model()
            else:
                os.remove(os.path.join(base_path, "models", "knn_model.pkl"))
                os.remove(os.path.join(base_path, "models", "data_baru.pkl"))
            return jsonify("Success delete"), 200
        else:
            return jsonify("FaceId tidak ditemukan"), 400
    except Exception as e:
        print("An error occurred:", str(e))
        return jsonify({"error": str(e)}), 400
...
if __name__ == "__main__":
	...
	context.load_cert_chain('/etc/letsencrypt/live/oneeda.my.id/cert.pem', '/etc/letsencrypt/live/oneeda.my.id/privkey.pem)
